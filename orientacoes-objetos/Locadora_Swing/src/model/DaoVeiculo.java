...
public class DaoVeiculo {
    ...
    public ArrayList<Veiculo> buscarTodos(){
        ...
        try {
            this.conectar();
            ResultSet rs = st.executeQuery("SELECT * FROM tb_veiculos ORDER BY marca;");
            while(rs.next()){
    ...
    public ArrayList<Veiculo> filtrar(String opcao, String filtro)
    {
        ArrayList<Veiculo> veiculosFiltrados = new ArrayList<Veiculo>();
        
        if(!opcao.equals("marca") && !opcao.equals("modelo")){
            return veiculosFiltrados;
        }
        try {
            this.conectar();
            ResultSet rs = st.executeQuery("SELECT * FROM tb_veiculos WHERE " + opcao + " LIKE '%" + filtro + "%'; ");
            while(rs.next()){
                Veiculo v = new Veiculo();
                v.setCodigo(rs.getInt("codigo"));
                v.setMarca(rs.getString("marca"));
